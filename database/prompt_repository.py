    ...
    DEFAULT_PROMPTS = {
        "profile_extraction": {
            ...
            "template": (
                ...
                "**IMPORTANT: ALL responses must be in SPANISH. Every field value must be written in Spanish.**\n\n"
                "**Core Principles:**\n"
                "1. **Accuracy First:** Extract ONLY verifiable information from the transcription. DO NOT invent years of experience, specific numbers, or details not mentioned.\n"
                "2. **Professional Depth:** Provide detailed, specific descriptions rather than generic statements\n"
                "3. **Intelligent Inference:** When information is implicit, use professional context to infer logically, but ALWAYS indicate it's an inference\n"
                "4. **Structured Output:** Maintain strict JSON format with complete, well-formed sentences\n"
                "5. **Language:** ALL field values MUST be in Spanish\n"
                "6. **No Fabrication:** If information is not mentioned, say \"No mencionado explícitamente\" - DO NOT make up details\n\n"
                ...
                "- Si no se menciona explícitamente pero la profesión requiere educación específica, inferir lógicamente:\n"
                "  * Ingeniero de Software → \"Probablemente tiene título en Ciencias de la Computación, Ingeniería de Software o campo relacionado (no mencionado explícitamente)\"\n"
                "  * Científico de Datos → \"Probablemente tiene título en Ciencias de la Computación, Estadística, Matemáticas o campo relacionado (no mencionado explícitamente)\"\n"
                ...
                "- Destacar proyectos exitosos con impacto\n"
                "- Mencionar roles de liderazgo o iniciativas lideradas\n"
                "- Si no se mencionan logros específicos, usar: \"No mencionado explícitamente en la presentación\"\n\n"
                "**Soft Skills:**\n"
                ...
                "- ALL field values MUST be in SPANISH (not English)\n"
                "- Return ONLY the raw JSON object - NO markdown, NO ```json``` blocks, NO explanations, NO additional text\n"
                "- Start your response directly with {{ and end with }}\n"
                "- Use complete sentences in SPANISH, not bullet points or fragments\n"
                "- Use \"No mencionado explícitamente\" instead of \"Not explicitly mentioned\"\n"
                "- Maintain professional, formal language in SPANISH throughout\n"
                "- Ensure all JSON strings are properly escaped (use \\ for quotes inside strings)\n"
                "- Do not invent information; if uncertain, indicate inference clearly in SPANISH\n"
                "- Your entire response must be valid, parseable JSON with ALL values in SPANISH\n\n"
                "**Text to analyze:**\n{text}\n\n"
        ...
        "cv_generation": {
            ...
            "template": (
                ...
                "**NO HACER:**\n"
                "- Usar frases genéricas como \"profesional dinámico\" u \"orientado a resultados\" sin contexto\n"
                "- Incluir información marcada como \"No especificado\" o \"No mencionado\"\n"
                "- Usar clichés o expresiones sobreutilizadas\n"
                "- Escribir en primera persona o usar el nombre del candidato\n"
                "- Agregar formato markdown, asteriscos o viñetas\n"
                "- Incluir texto de marcador de posición o corchetes\n"
                "- Hacer afirmaciones no respaldadas por los datos fuente\n"
                "- Exceder el conteo de palabras (mantenerse dentro de 180-220 palabras)\n\n"
                "**Quality Benchmarks:**\n"
